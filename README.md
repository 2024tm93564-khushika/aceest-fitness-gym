...
### Why certain files are excluded from version control
...
Each exclusion is intentional:

- **`*.db` (database files)** — `clients.db` is a runtime artifact, auto-generated by `init_db()` when the application first starts. Committing it would mean every developer sees a different data state, and it grows with every test run. The database *schema* is defined in `app.py` — that is what belongs in version control, not the data itself.

- **`__pycache__/` and `*.pyc`** — Python bytecode generated automatically by the interpreter at runtime. These files are machine-specific, regenerated on every run, and committing them creates unnecessary noise in the diff history with no benefit.

- **`venv/`** — The virtual environment contains all installed third-party packages. These are always recreated from `requirements.txt` via `pip install`, so committing them would bloat the repository with hundreds of megabytes of files that every developer already generates locally.

- **`.env`** — Reserved for local environment variable overrides (e.g. secret keys, custom DB paths). This file must never be committed as it can contain sensitive credentials.
...
# Expected: {"app": "ACEest Fitness API", "status": "ok"}
```
...
|---|---|---|
| GET | `/` | Returns API version string |
| GET | `/health` | Pipeline health check — returns `status: ok` |
| POST | `/clients` | Register or update a client |
| GET | `/clients` | Retrieve all clients |
| GET | `/clients/<name>` | Retrieve a single client by name |
| DELETE | `/clients/<name>` | Delete a client |
| POST | `/progress` | Log a client's weekly adherence |
| GET | `/progress/<name>` | Retrieve progress history for a client |
...
**Get a Single Client:**
```bash
curl http://localhost:5000/clients/Priya
```
...
tests/test_app.py::test_create_client_success      PASSED
tests/test_app.py::test_create_client_invalid      PASSED
tests/test_app.py::test_get_all_clients            PASSED
tests/test_app.py::test_get_client_found           PASSED
tests/test_app.py::test_get_client_not_found       PASSED
tests/test_app.py::test_delete_client              PASSED
tests/test_app.py::test_add_progress_success       PASSED
tests/test_app.py::test_get_progress_found         PASSED
tests/test_app.py::test_get_progress_not_found     PASSED
tests/test_app.py::test_add_progress_invalid       PASSED
tests/test_app.py::test_calories_fat_loss          PASSED
tests/test_app.py::test_calories_muscle_gain       PASSED
tests/test_app.py::test_calories_beginner          PASSED
tests/test_app.py::test_calories_invalid           PASSED
...
```bash
docker images aceest-fitness-gym
```

The `python:3.10-slim` base image keeps the final image size around **~150MB**, compared to ~900MB for the full Python image.
...
**Design decisions:**
- `python:3.10-slim` keeps the image significantly smaller than the full Python image (~150MB vs ~900MB)
- `--no-cache-dir` avoids storing pip's download cache in the image layer
- `requirements.txt` is copied and installed *before* source code — Docker layer caching then avoids reinstalling dependencies on every code change, only when `requirements.txt` itself changes
- `host="0.0.0.0"` in `app.run()` ensures Flask binds to all network interfaces inside the container, making it reachable through Docker's port mapping
...
| **Flask v3.2.4** | **REST API** | **Migration to Flask — this repository** |
